document.addEventListener("DOMContentLoaded", () => {
  ...
  if (projectsIndex !== -1) {
    pathParts.splice(projectsIndex, 1);
  }

  const repoName = pathParts.length > 0 ? pathParts[0] : null;
  ...
  const renderBreadcrumbs = () => {
    ...
    if (repoName) {
      parts.push({ name: repoName, link: `/projects/${repoName}` });
      ...
      if (currentPath) {
        ...
        pathSegments.forEach((segment) => {
          ...
          parts.push({
            name: segment,
            link: `/projects/${repoName}/${accumulatedPath}`,
          });
        });
      }
    }
  ...
  const renderDirectory = (items) => {
    ...
    if (currentPath) {
      ...
      const parentLink = `/projects/${repoName}${
    ...
    items.forEach((item) => {
      ...
      let linkUrl = "";
      if (!repoName) {
        linkUrl = `/projects/${item.name}`;
      } else {
        const nextPath = currentPath
          ? `${currentPath}/${item.name}`
          : item.name;
        linkUrl = `/projects/${repoName}/${nextPath}`;
      }
